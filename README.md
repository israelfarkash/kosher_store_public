...
1. העלה את [`apps.json`](/Users/israelfarkash/Downloads/חנות%20אפליקציות%20אונליין%20כשרה%20-%20מבוססת%20על%20דרייב/apps.json) לריפו ציבורי ב-GitHub.
2. שמור את האייקונים בתיקיית `icons/` באותו ריפו.
3. העלה את קבצי ה-APK ל-Google Drive והגדר לכל APK שיתוף `Anyone with the link` כ-Viewer.
4. עדכן ב-[app/build.gradle](/Users/israelfarkash/Downloads/חנות%20אפליקציות%20אונליין%20כשרה%20-%20מבוססת%20על%20דרייב/app/build.gradle) את `APPS_JSON_URL` לכתובת GitHub Raw האמיתית:

```text
https://raw.githubusercontent.com/USER/REPO/main/apps.json
```

5. ב-`apps.json`, שדה `apkUrl` צריך להצביע ל-Google Drive, ושדה `iconUrl` צריך להצביע ל-GitHub Raw.
